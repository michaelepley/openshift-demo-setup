...
[[ -v OPENSHIFT_RHTPSIO_USER_PASSWORD_DEFAULT_CIPHERTEXT ]] && echo "	--> Decrypting the script key" && { OPENSHIFT_RHTPSIO_USER_PASSWORD_DEFAULT_PLAINTEXT=`echo "${OPENSHIFT_RHTPSIO_USER_PASSWORD_DEFAULT_CIPHERTEXT}" | openssl enc -d -pbkdf2 -salt -a -aes-256-cbc -k ${SCRIPT_ENCRYPTION_KEY}` || { echo "FAILED: Could not validate the password" && exit 1; } ; }
