...
OPENSHIFT_APPS_DAN_REDHATGOVIO_PRIMARY=int.${OPENSHIFT_DOMAIN_DAN_REDHATGOVIO}
...
[[ -v OPENSHIFT_DAN_REDHATGOVIO_USER_PASSWORD_DEFAULT_PLAINTEXT ]] || [[ -v OPENSHIFT_DAN_REDHATGOVIO_USER_PASSWORD_DEFAULT_CIPHERTEXT ]] || { echo "FAILED: OPENSHIFT_DAN_REDHATGOVIO_USER_PASSWORD_DEFAULT_PLAINTEXT must be set and match a valid password for the openshift cluster ${OPENSHIFT_MASTER_DAN_REDHATGOVIO_PRIMARY}"  ; }
...
[[ -v OPENSHIFT_DAN_REDHATGOVIO_USER_PASSWORD_DEFAULT_CIPHERTEXT ]] && { OPENSHIFT_DAN_REDHATGOVIO_USER_PASSWORD_DEFAULT_PLAINTEXT=`echo "${OPENSHIFT_DAN_REDHATGOVIO_USER_PASSWORD_DEFAULT_CIPHERTEXT}" | openssl enc -d -a -aes-256-cbc -k "${SCRIPT_ENCRYPTION_KEY}"` || { echo "FAILED: Could not validate the password" && exit 1; } ; }
