...
[[ -v CONFIGURATION_DEMO_SETUP_TEST_RHTPSIO_COMPLETED ]] && echo "Using openshift demo setup test configuration" && { return || exit ; }
...
#OPENSHIFT_USER_PROJECT_REF="OPENSHIFT_USER_RHTPSIO_MEPLEY[3]" && eval "${OPENSHIFT_USER_PROJECT_REF}=mepley-test-setup"
: ${OPENSHIFT_PROJECT_TEST_SETUP_DEFAULT:=${OPENSHIFT_USER_RHTPSIO_MEPLEY[0]}-test-setup}
...
if [ "$CONFIGURATION_OPENSHIFT_SETUP_TEST_DISPLAY" != "false" ]; then
	...
	echo "	OPENSHIFT_USER_RHTPSIO_MEPLEY           = ${OPENSHIFT_USER_RHTPSIO_MEPLEY[@]}"
...
CONFIGURATION_DEMO_SETUP_TEST_RHTPSIO_COMPLETED=true
