...
[[ -v CONFIGURATION_DEMO_SETUP_TEST_MEPLEY_SHIMMY_REDHATGOVIO_COMPLETED ]] && echo "Using openshift demo setup test configuration" && { return || exit ; }
...
: ${OPENSHIFT_PROJECT_TEST_SETUP_DEFAULT:=${OPENSHIFT_USER_MEPLEY_SHIMMY_REDHATGOVIO_ADMIN[0]}-test-setup}
...
if [ "$CONFIGURATION_OPENSHIFT_SETUP_TEST_DISPLAY" != "false" ]; then
	echo "Demo Openshift Shimmy Configuration_________________________"
	echo "	OPENSHIFT_USER_REFERENCE_PRIMARY_DEFAULT = ${OPENSHIFT_USER_REFERENCE_PRIMARY_DEFAULT}"
	echo "	OPENSHIFT_USER_MEPLEY_SHIMMY_REDHATGOVIO_ADMIN           = ${OPENSHIFT_USER_MEPLEY_SHIMMY_REDHATGOVIO_ADMIN[@]}"
...
CONFIGURATION_DEMO_SETUP_TEST_MEPLEY_SHIMMY_REDHATGOVIO_COMPLETED=true
